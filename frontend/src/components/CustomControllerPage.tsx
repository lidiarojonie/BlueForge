...
export default function CustomControllerPage() {
    ...
    // 🔥 Si está logueado, pre-rellenamos el campo de creador con su nombre
    const [publishForm, setPublishForm] = useState({ 
        author_name: customer?.username || '', 
        build_name: '' 
    });
    ...
    // Actualizar nombre de creador si el usuario cambia / hace login
    useEffect(() => {
        if (customer?.username) setPublishForm(prev => ({ ...prev, author_name: customer.username }));
    }, [customer]);
    ...
    useEffect(() => {
        ...
        if (buildHash && dbParts.length > 0) {
            try {
                ...
                if (savedBuildIds.sh) setConfig({ joy_shape: savedBuildIds.sh });
            } catch (e) {
    ...
    const handlePublishToGallery = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                const newBuildId = data.build?.shared_build_id || data.build?.id;

                // 🔥 Guardamos en el navegador usando una llave ÚNICA por usuario
                if (newBuildId) {
                    const arsenalKey = customer ? `my_forge_builds_${customer.id}` : 'my_forge_builds_guest';
                    const myBuilds = JSON.parse(localStorage.getItem(arsenalKey) || '[]');
                    if (!myBuilds.includes(newBuildId)) {
                        myBuilds.push(newBuildId);
                        localStorage.setItem(arsenalKey, JSON.stringify(myBuilds));
                    }
                }

                alert('✨ Masterpiece published! Redirecting to the Gallery...');
                setShowPublishModal(false);
                setPublishForm(prev => ({ ...prev, build_name: '' }));
                
                navigate(`/gallery${newBuildId ? `?highlight=${newBuildId}` : ''}`);
            } else {
                ...
                return (
                    ...
                    >
                        <div className="w-12 h-12 rounded-full overflow-hidden mb-2 border border-white/10 bg-zinc-800 flex-shrink-0 shadow-inner relative">
                            {useColorSwatch ? (
                                <div className="w-full h-full group-hover:scale-110 transition-transform" style={{ backgroundColor: hexColor, boxShadow: 'inset 0 2px 4px rgba(0,0,0,0.5)' }} />
                            ) : (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 bg-zinc-900/50 p-8 rounded-[2rem] border border-white/10 shadow-2xl relative">
                    ...
                    <div className="flex flex-col">
                        <div className="flex flex-wrap gap-2 mb-8 border-b border-white/10 pb-4">
                            {[{ id: 'shell', label: 'Shell' }, { id: 'buttons', label: 'Buttons' }, { id: 'joysticks', label: 'Thumbsticks' }, { id: 'd_pad', label: 'D-Pad' }, { id: 'texture', label: 'Texture' }].map((tab) => (
                                <button
                                    key={tab.id}
                                    onClick={() => setActiveTab(tab.id as any)}
                                    className={`px-5 py-2 uppercase font-black text-sm tracking-widest transition-all rounded-xl ${activeTab === tab.id ? 'bg-cyan-500 text-black shadow-[0_0_15px_rgba(34,211,238,0.3)]' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
                                >
                                    {tab.label}
                                </button>
                            ))}
                        ...
                        <div className="flex-1 overflow-y-auto pr-2 scrollbar-hide">
                            {activeTab === 'shell' && <div className="animate-in fade-in slide-in-from-bottom-4 duration-300"><h3 className="text-xl font-bold mb-6">Choose your Shell</h3>{renderPartGrid(dbParts.filter(p => p.category === 'shell'), 'shell', false)}</div>}
                            {activeTab === 'buttons' && <div className="animate-in fade-in slide-in-from-bottom-4 duration-300"><h3 className="text-xl font-bold mb-6">Button Color</h3>{renderPartGrid(dbParts.filter(p => p.category === 'buttons'), 'buttons', true)}</div>}
                            {activeTab === 'd_pad' && <div className="animate-in fade-in slide-in-from-bottom-4 duration-300"><h3 className="text-xl font-bold mb-6">D-Pad Color</h3>{renderPartGrid(dbParts.filter(p => p.category === 'd_pad'), 'd_pad', true)}</div>}
                            {activeTab === 'texture' && <div className="animate-in fade-in slide-in-from-bottom-4 duration-300"><h3 className="text-xl font-bold mb-6">Special Pattern</h3>{renderPartGrid(dbParts.filter(p => p.category === 'texture'), 'texture', false)}</div>}
                            {activeTab === 'joysticks' && (
                                <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
                                    ...
                                    <div className="mb-8 border-b border-white/5 pb-8">
                                        ...
                                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                            {[{ key: 'Concavo-alto', label: 'High Concave' }, { key: 'Concavo-bajo', label: 'Low Concave' }, { key: 'Convexo-alto', label: 'High Convex' }, { key: 'Convexo-bajo', label: 'Low Convex' }].map(shape => (
                                                <button key={shape.key} onClick={() => setConfig({ ...config, joy_shape: shape.key })} className={`p-3 rounded-xl border-2 transition-all ${config.joy_shape === shape.key ? 'border-cyan-400 bg-cyan-400/10 text-cyan-400' : 'border-white/10 text-gray-400 hover:bg-white/5'}`}>
                                                    <span className="text-xs font-bold text-center block leading-tight">{shape.label}</span>
                                                </button>
                                            ))}
                                        </div>
                                    </div>
                                    <div className="mb-8"><p className="text-sm text-cyan-400 uppercase tracking-widest font-black mb-4">Base Color</p>{renderPartGrid(dbParts.filter(p => p.category === 'joysticks' && (p.name || p.color || '').toLowerCase().includes('base')), 'joystick_base', true)}</div>
                                    <div><p className="text-sm text-cyan-400 uppercase tracking-widest font-black mb-4">Top Color</p>{renderPartGrid(dbParts.filter(p => p.category === 'joysticks' && (p.name || p.color || '').toLowerCase().match(/seta|mushroom|mu/)), 'joystick_mushroom', true)}</div>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="mt-8 pt-6 border-t border-white/10 flex flex-col sm:flex-row justify-between items-center gap-6">
                            <div className="w-full sm:w-auto text-center sm:text-left">
                                ...
                                <p className="text-4xl font-black text-cyan-400 drop-shadow-[0_0_10px_rgba(34,211,238,0.5)]">€{totalPrice.toFixed(2)}</p>
                            </div>
                            
                            <div className="flex gap-3 w-full sm:w-auto">
                                <button onClick={() => setShowPublishModal(true)} title="Publish to Forge Gallery" className="p-4 bg-fuchsia-600 hover:bg-fuchsia-500 text-white rounded-2xl transition-all border border-white/10 flex items-center justify-center hover:scale-[1.05] shadow-[0_0_20px_rgba(192,38,211,0.4)]">
                                    <Globe size={24} />
                                </button>
                                <button onClick={handleAddToCart} className="flex-1 sm:flex-none px-6 py-4 bg-cyan-500 hover:bg-cyan-400 text-black font-black text-lg rounded-2xl uppercase tracking-widest flex items-center justify-center gap-3 transition-all hover:scale-[1.02] shadow-[0_0_20px_rgba(34,211,238,0.3)]">
                    ...
                    {showPublishModal && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in p-4">
                            <div className="bg-zinc-900 border border-white/10 p-8 rounded-3xl shadow-2xl max-w-md w-full relative animate-in zoom-in-95">
                                <button onClick={() => setShowPublishModal(false)} className="absolute top-6 right-6 text-gray-500 hover:text-white transition-colors"><X size={24} /></button>
                                <h2 className="text-2xl font-black uppercase text-white mb-2 flex items-center gap-3 tracking-tight"><Globe className="text-fuchsia-500" /> Publish Build</h2>
                                ...
                                <form onSubmit={handlePublishToGallery} className="space-y-5">
                                    <div>
                                        ...
                                        <input required type="text" maxLength={30} placeholder="e.g. Neon Demon, Stealth Pro..." value={publishForm.build_name} onChange={e => setPublishForm({...publishForm, build_name: e.target.value})} className="w-full bg-black/50 border border-white/10 rounded-xl p-4 text-white focus:border-fuchsia-500 focus:outline-none focus:ring-1 focus:ring-fuchsia-500 transition-colors" />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Creator Name</label>
                                        <input required type="text" maxLength={20} placeholder="Your Gamertag..." value={publishForm.author_name} onChange={e => setPublishForm({...publishForm, author_name: e.target.value})} className="w-full bg-black/50 border border-white/10 rounded-xl p-4 text-white focus:border-fuchsia-500 focus:outline-none focus:ring-1 focus:ring-fuchsia-500 transition-colors" />
                                    </div>
                                    <button type="submit" disabled={isPublishing} className="w-full py-4 mt-2 bg-gradient-to-r from-fuchsia-600 to-purple-600 hover:from-fuchsia-500 hover:to-purple-500 text-white font-black text-sm uppercase tracking-widest rounded-xl transition-all disabled:opacity-50 shadow-[0_0_20px_rgba(192,38,211,0.3)]">
